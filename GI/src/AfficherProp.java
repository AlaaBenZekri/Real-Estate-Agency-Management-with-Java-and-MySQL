...
public class AfficherProp extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AfficherProp frame = new AfficherProp();
	...
	private void mjProp(ActionEvent e) {
		try {
			Connection con = MysqlConnection.con();
			String sql="UPDATE proprietaire SET Nom= ?, Prenom= ?, NumRue= ?, Rue= ?, CodePostale= ?, Localite= ?, NumTelPrv= ?, HeureDebPrsTelPrv= ?, HeureFinPrsTelPrv= ?, HeureDebPrsTelTrv= ?, HeureFinPrsTelTrv= ?  WHERE idProp= ? ;";
			...
			pstmt.setInt(7, Integer.parseInt(textNumTelP.getText()));
			pstmt.setInt(8, (int)de1.getValue());
			pstmt.setInt(9, (int)jus1.getValue());
			pstmt.setInt(10, (int)de2.getValue());
			pstmt.setInt(11, (int)jus2.getValue());
			pstmt.setInt(12, idProp);
			pstmt.execute();
			con.close();
